...
import { CheckCircle, XCircle, AlertCircle, Mail, Clock, X } from 'lucide-react'
...
export const VerificationToast: React.FC<VerificationToastProps> = ({
  ...
  const getIcon = () => {
    ...
    switch (notification.type) {
      ...
      case 'warning':
        return <AlertCircle {...iconProps} className="h-5 w-5 flex-shrink-0 text-orange-500" />
      case 'info':
        return <Mail {...iconProps} className="h-5 w-5 flex-shrink-0 text-blue-500" />
      default:
        return <AlertCircle {...iconProps} className="h-5 w-5 flex-shrink-0 text-gray-500" />
    }
  }
...
export const useToastNotifications = () => {
  ...
  const showResendError = (errorMessage?: string) => {
    return showError(
      ...
      errorMessage || 'We couldn\'t send the verification email. Please try again later.',
      {
        action: {
          label: 'Retry',
          onClick: () => window.location.reload(),
