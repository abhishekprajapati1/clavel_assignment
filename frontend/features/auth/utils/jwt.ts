 ...
 */
export const validateEmailVerificationToken = (token: string): TokenValidationResult => {
  const result = decodeJWTToken(token)
...
export const getJWTErrorMessage = (error: string): string => {
  const errorMap: Record<string, string> = {
    'Invalid token format': 'The verification link appears to be malformed. Please check the link or request a new one.',
    'Failed to decode token': 'Unable to read the verification token. The link may be corrupted.',
    'Token does not contain email': 'The verification link is missing required information.',
    'Token is not a verification token': 'This link is not a valid email verification link.',
  }

  return errorMap[error] || 'An error occurred while processing the verification link.'
}
...
export const debugToken = (token: string): void => {
  ...
  if (result.payload?.exp) {
    ...
    console.log('Time remaining:', formatTimeRemaining(getTimeUntilExpiration(token)))
  }
